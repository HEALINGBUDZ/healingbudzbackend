...
(function ($, _, undefined) {
    ...
    var MentionsInput = function (input) {
        ...
        function initTextarea() {
            ...
            elmInputBox.bind('keyup', onInputBoxKeyDown);
            elmInputBox.bind('input paste', onInputBoxKeyPress);
//            elmInputBox.bind('keypress', onInputBoxKeyPress);
        ...
        $(document.body).delegate('.mention', 'keypress', function (e) {
...
//            elmAutocompleteList.delegate('li', 'keypress', onAutoCompleteItemClick);
        }
...
//                addMention(mention.value,mention.id,mention.type)
        ...
        function addMention(value, id, type) {
            ...
            if ('selectionStart' in input) {
                ...
                selection = sel.text.length - selLen;
            }

//            var startCaretPosition = regex.lastIndex - currentDataQuery.length - 1;
            //var currentCaretPosition = regex.lastIndex;
            var currentCaretPosition = selection;
            var startCaretPosition = currentCaretPosition - currentDataQuery.length - 1;

            var start = currentMessage.substr(0, startCaretPosition);
            var end = currentMessage.substr(currentCaretPosition, currentMessage.length + startCaretPosition);
            ...
            pre_append = '#';
            if (type == 'user' || type == 'budz') {
                pre_append = '@';
            }
            value = pre_append + value;
            ...
            var comment_id = elmInputBox.attr('id');
            var comment_class = elmInputBox.attr('class');
            if (comment_class == 'comments-mention') {
                $('#' + comment_id).val($('#' + comment_id).val() + ' ');
            }
//             elmInputBox.val(updatedMessageText+' ');
//            ($('.comments-mention').val($('.comments-mention').val()+' '));
        }
        ...
        function onAutoCompleteItemClickLoad() {
            var elmTarget = $(this);
            addMention(elmTarget.attr('data-display'), elmTarget.attr('data-ref-id'), elmTarget.attr('data-ref-type'));
...
//            var typedValue = String.fromCharCode(e.which || e.keyCode);
            var typedValue = e.originalEvent.data;
            if (typedValue) {
                if (typedValue.length > 1) {
                    typedValue = typedValue.substr(typedValue.length - 1);
                }
        ...
        function onInputBoxKeyDown(e) {
            ...
            if (e.keyCode == KEY.BACKSPACE) {
                inputBuffer = inputBuffer.slice(0, inputBuffer.length - 1); // Can't use splice, not available in IE
...
if (elmActiveAutoCompleteItem && elmActiveAutoCompleteItem.length) {
        ...
        return {
            ...
            updateValuesStyle: function (id, val, type) {
                addMention(id, val, type);
