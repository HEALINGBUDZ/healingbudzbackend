...
if (count($posts) > 0) {
    ...
    ?>
    <?php foreach ($posts as $key => $post) {  if ($post->scrapedUrl && count($post->files) == 0 && strpos($post->scrapedUrl, 'get-question-answers') !== false ) { ?>
        <div id="single-post-<?= $post->id; ?>">
                ...
                <div class="wall-post-single-in-body">
                    ...
                    <?php if ($post->shared_id) { ?>
                        <div class="hb_repost_comments">
                            ...
                            <?php } ?>
                                <div class="qa-repost-section-wall">
                                    <figure><img src="<?php echo asset('userassets/images/QA-scrap-in-app.png') ?>" alt="QA"></figure>
                                    <h1><?= $post->scrapedUrl->title?></h1>
                                    <p><?= $post->scrapedUrl->content ?></p>
                                    <div class="qa-btn-wall-area">
                                        <a href="<?= $post->scrapedUrl->extracted_url ?>" class="wall-join-blue">Join Discussion</a>
                                        <?php  $id=explode('/',$post->scrapedUrl); $answers_count=getQuestoinsAnswers( end($id));if($answers_count > 0){ ?>
                                        <a href="<?= $post->scrapedUrl->extracted_url ?>" class="wall-answer-blue">
                                            <span><?php echo $answers_count;?></span>
                                            Answers
                                        </a>
                                        <?php } else{ ?>
                                         <a href="<?= $post->scrapedUrl->extracted_url ?>" class="wall-answer-blue">Be the 1<sup>st </sup> Bud to answer</a>   
                                        <?php } ?>
            ...
            <div  style="display: block" class="wall-post-single-body" id="wall-post-single-body-<?= $post->id?>">
                <?php if(Auth::user()){ ?>
                ...
                <div class="wall-post-single-in-body">
                    ...
                    <?php if ($post->shared_id) { ?>
                        <div class="hb_repost_comments">
                            <div class="hb_repost_comments_txt">  
                            <?php } ?>
                            <ul class="list-none post_gallery_thumbs <?php if (count($post->files) == 1) { ?> one-post-image<?php } ?>" >
                                <?php
                                $count = 1;
                                foreach ($post->files as $fileKey => $file) {
                                    ...
                                    <li <?= (($count > 1) ? 'style="display:none"' : '') ?>>
                                        ...
                                        <?= ($count < 1) ? '<a href="' . (($file->type == 'video') ? '#video_' . $post->id : asset(image_fix_orientation('public/images' . $file->file)) ) . '" data-fancybox="gallery' . $post->id . '" >' : ''; ?>
                            ...
                            <figcaption>
                                ...
                                <?php if ($post->scrapedUrl && count($post->files) == 0) { ?>
                                    <div class="scraped-data">
                                        <?php 
                                        if(strpos($post->scrapedUrl, 'strain-details') !== false ) { ?>
                                        <img src="<?= asset('userassets/images/Strains_scrape.png') ?>" alt="">
                                        <?php }else{
                                        ?>
                                        <img src="<?= $post->scrapedUrl->image; ?>" alt="">
                                        <?php } ?>
                                        <h4><a href="<?= $post->scrapedUrl->url; ?>" target="_blank"><?= $post->scrapedUrl->title ?></a></h4>
                ...
                <div class="popup-area">
                    <form action="<?php echo asset('report-single-post'); ?>" class="reporting-form" method="post" >
                        ...
                        <fieldset>
                            ...
                            <input class="<?= 'reason_' . $post->id ?>" type="radio" name="reason" id="sexual<?= $post->id ?>" checked  value="Nudity or sexual content">
                            <label for="sexual<?= $post->id ?>">Nudity or sexual content</label>

                            <input class="<?= 'reason_' . $post->id ?>" type="radio" name="reason" id="harasssment<?= $post->id ?>"  value="Harassment or hate speech">
                            <label for="harasssment<?= $post->id ?>">Harassment or hate speech</label>

                            <input class="<?= 'reason_' . $post->id ?>" type="radio" name="reason" id="threatening<?= $post->id ?>"  value="Threatening, violent, or concerning">
